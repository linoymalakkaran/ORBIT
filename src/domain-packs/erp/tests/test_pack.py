...
def test_purchase_order_skill_exists():
    ids = [s["id"] for s in get_domain_manifest()["skills"]]
    assert "erp-purchase-order" in ids


def test_compliance_includes_ifrs():
    m = get_domain_manifest()
    assert any("IFRS" in c for c in m["compliance"])
