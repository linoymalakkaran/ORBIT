"""Tests for the PCS (Port Community System) domain pack."""
...
def test_customs_declaration_skill_exists():
    ids = [s["id"] for s in get_domain_manifest()["skills"]]
    assert "pcs-customs-declaration" in ids


def test_compliance_includes_wco():
    m = get_domain_manifest()
    assert any("WCO" in c for c in m["compliance"])
